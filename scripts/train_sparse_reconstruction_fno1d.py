...
from src.models.fno1d.fno1d import build_fno1d_model  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="Train FNO1D for sparse trajectory reconstruction."
    )
    parser.add_argument("--dataset-path", type=Path, required=True)
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("outputs/sparse_reconstruction_fno1d"),
    )
...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    for name in ("epochs", "batch_size", "modes", "width", "depth"):
...
def _replace_last_checkpoint(
    ...
    """在本次新建 run directory 内更新最后一个 checkpoint。"""
    torch.save(
        {
            "epoch": int(epoch),
            "best_val_relative_l2": float(best_val_relative_l2),
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if output_dir.exists():
        raise FileExistsError(
            f"Output run directory already exists and will not be overwritten: {output_dir}"
        )
    ...
    run_config: dict[str, Any] = {
        "schema_version": "1.0",
        "experiment_type": "sparse_trajectory_reconstruction_fno1d",
    ...
    print("Sparse FNO1D reconstruction training completed.")
