...
from src.models.timesnet1d import build_timesnet1d_model  # noqa: E402
...
def build_parser() -> argparse.ArgumentParser:
    """构造稀疏 TimesNet1D 重建训练命令行。"""
    parser = argparse.ArgumentParser(
        description="Train canonical TimesNet1D for sparse trajectory reconstruction."
    )
    parser.add_argument("--dataset-path", type=Path, required=True)
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("outputs/sparse_reconstruction_timesnet1d"),
    )
...
def train_one_epoch(
    ...
    for batch in loader:
        ...
        loss = hidden_only_mse_loss(prediction, target_normalized, hidden_mask)
        loss.backward()
        optimizer.step()
...
def run_training(args: argparse.Namespace) -> dict[str, Any]:
    ...
    run_config: dict[str, Any] = {
        ...
        "training": {
            ...
            "seed": int(args.seed), "optimizer": "AdamW", "lr": float(args.lr),
            "weight_decay": float(args.weight_decay), "scheduler": "ExponentialLR",
            "scheduler_gamma": float(args.scheduler_gamma), "device": str(args.device),
        },
    ...
    history: dict[str, list[float]] = {
        "train_normalized_hidden_mse": [], "val_normalized_hidden_mse": [],
        "val_raw_hidden_relative_l2": [], "lr": [],
    }
    ...
    _save_json_new(logs_dir / "train_summary.json", {
        "best_epoch": int(best_epoch),
        "best_validation_raw_hidden_relative_l2": best_val_relative_l2,
        "test_normalized_hidden_mse": float(test_loss), "training_seconds": float(elapsed_seconds),
    })
    _save_json_new(metrics_dir / "test_hidden_only_metrics.json", {
        "split": "test", "model_space_hidden_mse": float(test_loss),
        "raw_hidden_only_metrics": test_metrics,
    })
    summary = {
        "output_dir": str(output_dir), "best_epoch": int(best_epoch),
        "best_validation_raw_hidden_relative_l2": best_val_relative_l2,
        "test_normalized_hidden_mse": float(test_loss),
        "test_raw_hidden_relative_l2": float(test_metrics["overall"]["relative_l2"]),
    }
    print("Sparse TimesNet1D reconstruction training completed.")
